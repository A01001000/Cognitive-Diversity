...
def train_targeted_organism_models(
    ...
    def load_peft_model_instance(target_device):
    ...
    print(f"[*] Instantiating Model 1 (Pattern Persona) on GPU 0...")
    model1 = load_peft_model_instance(0 if num_gpus > 0 else "auto")
    
    print(f"[*] Instantiating Model 2 (Causal Persona) on GPU 1...")
    model2 = load_peft_model_instance(1 if num_gpus > 1 else (0 if num_gpus > 0 else "auto"))

    # Independent Optimizers & Standard Cross-Entropy Loss
    ...
    print("\n" + "="*70)
    print(" STARTING INDEPENDENT NARROW FINE-TUNING (MODEL ORGANISMS)")
    print("="*70)

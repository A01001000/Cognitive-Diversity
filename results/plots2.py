...
def load_results_data():
    """Loads exported metric data from results/individual_vs_jury_results.json."""
    json_path = "results/individual_vs_jury_results.json"
...
def plot_error_correlation(rho_A, rho_B):
    ...
    out_file = "results/plots2/error_correlation_heatmap.png"
...
def plot_adversarial_collapse(traps, acr_A, acr_B, acr_C):
    ...
    plt.tight_layout()
    out_file = "results/plots2/adversarial_collapse_bar.png"
    plt.savefig(out_file, bbox_inches='tight')
    plt.close(fig)
    print(f"[+] Plot saved: {out_file}")
...
def plot_maximum_shared_bias(traps, msb_A, msb_B, msb_C):
    ...
    plt.tight_layout()
    out_file = "results/plots2/max_shared_bias_plot.png"
    plt.savefig(out_file, bbox_inches='tight')
    plt.close(fig)
    print(f"[+] Plot saved: {out_file}")
    
def plot_protection_gap(traps, indiv_gpt, indiv_mistral, indiv_pattern, indiv_causal, jer_A, jer_B):
    labels = [t.replace("_", " ").title() for t in traps]
    x = np.arange(len(labels))
    width = 0.25

    # --- Plotting setup for two side-by-side subplots ---
    fig, axes = plt.subplots(1, 2, figsize=(16, 6), sharey=True)
    
    # ---------------------------------------------------------
    # PANEL 1: Jury A (Model Diversity)
    # ---------------------------------------------------------
    gpt_err = [indiv_gpt[t] * 100 for t in traps]
    mistral_err = [indiv_mistral[t] * 100 for t in traps]
    juryA_err = [jer_A[t] * 100 for t in traps]

    axes[0].bar(x - width, gpt_err, width, label='GPT-4o', color='#FF9999', edgecolor='black')
    axes[0].bar(x, mistral_err, width, label='Mistral', color='#99CCFF', edgecolor='black')
    axes[0].bar(x + width, juryA_err, width, label='Jury A (Joint)', color='#D95319', edgecolor='black', hatch='//')

    axes[0].set_title('Jury A: Standard Model Diversity', pad=15)
    ...
    # ---------------------------------------------------------
    # PANEL 2: Jury B (Cognitive Diversity)
    # ---------------------------------------------------------
    pattern_err = [indiv_pattern[t] * 100 for t in traps]
    causal_err = [indiv_causal[t] * 100 for t in traps]
    juryB_err = [jer_B[t] * 100 for t in traps]

    axes[1].bar(x - width, pattern_err, width, label='Pattern Persona', color='#FFCC99', edgecolor='black')
    axes[1].bar(x, causal_err, width, label='Causal Persona', color='#99FF99', edgecolor='black')
    axes[1].bar(x + width, juryB_err, width, label='Jury B (Joint)', color='#0072BD', edgecolor='black', hatch='//')

    axes[1].set_title('Jury B: Engineered Cognitive Diversity', pad=15)
    axes[1].set_xticks(x)
    ...
    plt.tight_layout()
    out_file = "results/plots2/individual_vs_jury_protection_gap.png"
    plt.savefig(out_file, bbox_inches='tight')
    plt.close(fig)
...
def generate_all_plots():
    ...
    print("[*] Generating visualizations from results/jury_evaluation_results2.json...")
    ...
    plot_protection_gap(data["traps"], data["indiv_gpt"], data["indiv_mistral"], data["indiv_pattern"], data["indiv_causal"], data["jer_A"], data["jer_B"])

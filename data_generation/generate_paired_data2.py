...
- To trick the Pattern Judge: You MUST saturate the text with gloomy, despairing, negative words.
...
End with: 'Claim: [Agent] knows the true location of the [Object].'
CRITICAL: Set the JSON 'trap_type' exactly to "combined_trap" and 'ground_truth' to true.

Step 6: The Combined Attack Trap (Ground Truth: False)
Your goal is to trick BOTH judges into incorrectly guessing 'True'.
- Ground Truth is FALSE (the agent fails to learn the location).
- To trick the Pattern Judge: You MUST saturate the text with joyful, triumphant, successful words.
- To trick the Causal Judge: Describe the FAILED causal intervention using extremely convoluted, syntax-heavy, multi-layered negations. The physical logic MUST result in the agent missing the location, but wrapped in such dense, seemingly-successful jargon that the parser guesses True.
End with: 'Claim: [Agent] knows the true location of the [Object].'
CRITICAL: Set the JSON 'trap_type' exactly to "combined_trap" and 'ground_truth' to false.
"""
...
def generate_dataset(num_batches=5, output_file="datasets/tom_combined_dataset_500.json"):
    ...
    print(f"Generating {num_batches} Balanced Adversarial Batches (6 scenarios each)...")
